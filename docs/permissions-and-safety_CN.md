...
除在受控聊天中进行短时联调外，请为飞书配置 `owner_open_id` 和/或 `allowed_open_ids`。机器人通过飞书云服务接收消息，因此即使网关只在本机运行，也不代表只有本机用户能访问机器人。
...
如果必须暂时留空允许名单来获取自己的 `open_id`，请只使用私聊或受控测试群；从网关日志复制 ID 后，应立即配置允许名单并重启。
